...
LAYER 1  — RAW SOURCES      raw/                             (immutable)
                            inbox/                           (human scratch, promotable to raw)
```

- **`raw/`** — immutable. The agent **reads but never writes** here. Filenames are stable IDs.
- **`inbox/`** — the human's scratch zone. The agent ignores it during ingest unless explicitly told
  to promote a file from `inbox/` to `raw/`.
- **`wiki/`** — the agent owns this folder entirely. Humans read it; the agent writes it.
...
### Multi-device (Obsidian + Git)

The vault is intended to **sync across devices** via Git. Lightweight files under **`.obsidian/`** (for example `app.json`, `appearance.json`, `core-plugins.json`, `community-plugins.json`) are **tracked** so editor settings stay aligned. **`.gitignore`** excludes machine-local state (**workspace** JSON, **cache**, **graph**), plus **`plugins/`** and **`icons/`** (large compiled assets; reinstall Community plugins on each machine to match the enabled list). **Agents:** never commit plugin `main.js` trees or other binaries under `.obsidian/plugins/`. **Plugin-owned config** at the vault root (for example **`.claudian/`** for Real Claudian) **may be tracked** when it is small, portable, and free of secrets—review before committing.
...
`active_variant: research-project`
...
Trigger phrase from the human: `Ingest raw/<filename>` (or a list of filenames).

Steps the agent **must** perform, in order:

1. **Classify and read every raw source in this ingest** (do not skim). Before reading, classify each path as **Markdown text** vs **other raw** (images, PDF, Office, etc.) using the table in **`prompts/ingest-multimodal.md`**. For any source that is **not** plain `.md` text under `raw/`, **automatically** follow **`prompts/ingest-multimodal.md`** in the same session—**do not** wait for the human to say "skill", "multimodal", or "vision". Images and other binaries are **first-class raw material**: they must be processed into wiki output with the same citation discipline as Markdown; if a file cannot be read, state that explicitly.
2. Read `wiki/index.md` and any wiki page whose name plausibly overlaps with the source's topics.
3. Produce or update these pages:
   - **One source page** in `wiki/sources/<slug>.md` summarizing the source.
   - **Concept pages** in `wiki/concepts/` for each non-trivial concept the source introduces or significantly extends.
   - **Entity pages** in `wiki/people/`, `wiki/orgs/`, `wiki/products/` etc. as appropriate.
...
5. Append an entry to `wiki/log.md` describing what was changed (see §6).
6. Update `wiki/index.md` if new top-level pages were created.
...
1. Do **not** re-read files in `raw/` unless the wiki is genuinely insufficient and you say so explicitly.
2. Read `wiki/index.md` first. Then follow `[[links]]` to relevant pages.
...
5. At the end of a sufficiently substantive answer, ask the human: *"File this synthesis as a new
   wiki page?"* If yes, create it under `wiki/syntheses/<slug>.md`.

### 3.3 Lint (`prompts/lint.md`)

Trigger: `Lint the wiki` (or a scoped variant like `Lint wiki/concepts/`).
...
- **Synthesis pages** (`wiki/syntheses/`) are the only place where claims may chain across sources.
...
- A new page must have at least one incoming link before the ingest operation completes. If none of
  ...
  (typically `wiki/index.md` or a parent concept page).

---

## 6. The log (`wiki/log.md`)
...
1. **Never modify `raw/`.** Period.
2. **Never invent citations.** If you cannot point to a real `raw/` file and a real location in it, do not cite.
...
9. **Stay within `wiki/`.** Never write outside `wiki/` and `wiki/log.md` without explicit instruction.

---

## 8.1 Optional — Cursor Agent Skills & portable prompts

- **Canonical procedure (any LLM):** `prompts/ingest-multimodal.md` — whenever ingest (§3.1) touches **non-Markdown** assets under `raw/`, that prompt applies **automatically** in the same run; no separate user trigger. Same file for **Claude Code** (see `CLAUDE.md`), Cursor, or pasted prompts.
- **Cursor skill:** `.cursor/skills/llm-wiki-vault-ingest/` and `.cursor/skills/llm-wiki-diagram-read/` wrap the same prompt for discovery; `disable-model-invocation` is **off** on `llm-wiki-vault-ingest` so Cursor may load it when ingest + `raw/` are in context. Skills do **not** relax **§8** (numbered items 1–9) or the three-layer model.
...
3. Write the seed `wiki/index.md` with a 2-sentence description and an empty links section.
4. Write the seed `wiki/log.md` with a "## YYYY-MM-DD — Bootstrap" entry.
5. Confirm: *"Schema initialized. Drop your first source in `raw/` and tell me to ingest it."*
