...
If you use **Obsidian** on more than one machine, keep this folder in a **Git** repo: `raw/`, `wiki/`, and lightweight **`.obsidian/*.json`** travel with commits; **plugin binaries** (the `.obsidian/plugins/` tree) stay **out of Git** (see `.gitignore`). After `git pull` on a new device, open Obsidian and install or update **Community plugins** so they match your `community-plugins.json` (Marketplace / “Check for updates”). Workspace layout and caches stay local. Small **plugin config** folders at the vault root (e.g. **`.claudian/`** for Real Claudian) are **tracked** so sessions/settings sync across devices—avoid committing secrets inside them.
