...
> Adapter Cursor della skill `retrieval-protocol` (EP-045, retrieval vivo e citato del tutor).

Skill fondante per il tutor EP-045. Ogni affermazione del tutor su un concetto documentato o su un componente del codebase deve essere ancorata a un artefatto reale recuperato al momento della risposta. Il retrieval non usa cache di sessione precedente: opera sempre sulla versione corrente degli artefatti (vedi §Vincoli).

Riferimenti: [wiki-search-protocol](mdc:.cursor/skills/wiki-search-protocol/SKILL.md) (EP-042, opt-in Fase 2), PATTERN §31 (Hybrid Wiki Search), [wiki-gap-protocol](mdc:.cursor/skills/wiki-gap-protocol/SKILL.md) (apertura lacune), PATTERN §6 (citazione).
...
2. Per classificazione `ambigua`: applica entrambe le fasi 2 e 3; in Fase 4 consolida i risultati da entrambe le sorgenti.
...
**Estrai i termini chiave** dalla domanda originale (max 3-5 token rilevanti, escludi stop-word).

### Branch EP-042 ON (`wiki_search.enabled: true` in `factory.config.yaml`)

Invoca [wiki-search-protocol](mdc:.cursor/skills/wiki-search-protocol/SKILL.md) con la query originale (o i termini chiave se la query è lunga). La skill restituisce una lista ordinata per rilevanza.
...
**Estratto pattern** dalla domanda (nome file, nome funzione, nome classe, modulo, path parziale).

Esegui glob ricorsivo sul repo (path configurato come `code_path` in `factory.config.yaml`, default `.`):
...
I campi `function` e `class` sono opzionali (presenti solo se il match individua con certezza una funzione o una classe specifica).
...
Formula la risposta usando il contenuto estratto. Apponi la citazione esplicita per ogni affermazione derivata da un artefatto wiki:
...
Formula la risposta descrivendo la struttura trovata. Cita il percorso file e, se disponibili, la funzione o la classe:
...
### Caso C — Hit da entrambe le sorgenti (classificazione `ambigua`)

Formula la risposta integrando le due sorgenti. Apponi citazioni wiki per i claim concettuali (formato `[^src: ...]`) e riferimenti file per i claim implementativi.
...
Appendi il gap-record a `wiki/gaps.md` (append-only, compatibile con il formato esistente del file).
...
I due schemi definiscono il contratto strutturato tra `retrieval-protocol` (produttore) e [tutor](mdc:.cursor/rules/tutor.mdc) / [epistemic-tag-protocol](mdc:.cursor/skills/epistemic-tag-protocol/SKILL.md) (consumatori). Ogni citazione deve essere derivata dagli output delle Fasi 2 e 3.
...
- `excerpt`: estratto testuale, max 200 caratteri. Opzionale.
...
code_citation:
  file: ".cursor/skills/retrieval-protocol/SKILL.md"  # path relativo al repo — obbligatorio
...
- `function`: nome della funzione specifica, se il retrieval individua uno scope funzione. Opzionale.
- `class`: nome della classe specifica, se il retrieval individua uno scope classe. Opzionale.

### Serializzazione in risposta (formato footnote markdown)

Il tutor serializza le citazioni come footnote markdown in coda all'affermazione o al paragrafo da cui derivano:
...
- Formato code: `[^code: <file>::<class>.<function>]` (se `class` e `function` entrambi presenti); `[^code: <file>::<function>]` se solo funzione; `[^code: <file>]` se solo file.
...
> (TSK-351) e classificate come non-conformità.
...
I marker segnalano l'esito del retrieval e sono emessi nel log o nel contesto di sessione per tracciabilità.
...
Campi obbligatori:
- `concept`: termine o concetto principale estratto dalla domanda. Stringa breve (max 80 char), non il testo completo della query.
...
1. **Freshness garantita da lettura filesystem corrente**: il retrieval opera sempre sulla versione corrente degli artefatti (`wiki/**`, `<code_path>/**`). Nessuno snapshot congelato di sessione precedente è usato come sorgente. Ogni esecuzione legge i file al momento della query.
2. **Nessun contenuto non ancorato**: se il tutor non trova un artefatto che supporta un'affermazione, dichiara la lacuna (Caso D, Fase 4). Mai generare risposte plausibili ma non tracciate a una sorgente reale.
3. **Citazioni verbatim dal testo recuperato**: il contenuto restituito dalla Fase 2 o 3 è citato fedelmente; il tutor può parafrasare solo se la citazione è esplicitamente attribuita (`[^src: ...]`).
4. **Gap-record append-only**: `wiki/gaps.md` è append-only. Mai sovrascrivere entry esistenti.
5. **Classificazione `ambigua` → doppia ricerca**: per domande cross-dominio non scartare una sorgente in anticipo; esegui entrambe le fasi (2 e 3).
