...
> Adapter Cursor della skill `temporal-estimate-protocol`.
...
Riferimenti: `wiki/syntheses/temporal-awareness-multiagent-patterns.md §Layer 3 — Coordinazione real-time`, `factory.config.yaml.temporal.estimate_protocol.*`, [temporal-budget-governor](mdc:.cursor/skills/temporal-budget-governor/SKILL.md) (ordine di invocazione).
...
**Nota**: se `completed_steps == 0` ma `elapsed_ms > 0`, il chiamante potrebbe avere già consumato budget senza avanzare. Il return `recommendation: continue` riflette che non ci sono basi per escalare — ma il chiamante può a sua discrezione verificare il vincolo economico via [temporal-budget-governor](mdc:.cursor/skills/temporal-budget-governor/SKILL.md) (vedi §Complementarietà).
...
**Ordine raccomandato**: invocare prima [temporal-budget-governor](mdc:.cursor/skills/temporal-budget-governor/SKILL.md), poi `temporal-estimate-protocol`:
...
Il vincolo economico ([temporal-budget-governor](mdc:.cursor/skills/temporal-budget-governor/SKILL.md)) ha **precedenza assoluta** sul vincolo temporale (`temporal-estimate-protocol`):
...
Questo schema preserva la semantica del [temporal-budget-governor](mdc:.cursor/skills/temporal-budget-governor/SKILL.md) come circuito di protezione principale ([[circuit-breaker]]) e posiziona `temporal-estimate-protocol` come sensore di velocità advisory.
