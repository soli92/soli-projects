...
> Adapter Cursor della skill `temporal-budget-governor` definita in PATTERN.md §3.

> **2° asse di terminazione del loop evaluator-optimizer** — bound economico complementare a `max_iterations` strutturale. Invocata inline dall'orchestrator a ogni step decisionale (review iteration, dev-agent retry, premortem deep-dive, wave dispatch). Comunica un verdict, **non lo esegue** (separation of concerns — pattern parallelo a [code-reviewer](mdc:.cursor/rules/code-reviewer.mdc) CQRL R.Q2). [[circuit-breaker]] applicato al costo.
...
- Wave plan §18.6 prodotto da [parallel-scheduling](mdc:.cursor/skills/parallel-scheduling/SKILL.md) (TSK-110), con i campi `token_budget` / `elapsed` / `estimated_remaining` / `bootstrap_mode`.
...
Il governor è **read-only** sul wave plan ([parallel-scheduling](mdc:.cursor/skills/parallel-scheduling/SKILL.md) è l'unico writer dei campi
...
Legge da:
1. Wave plan §18.6 (prodotto da [parallel-scheduling](mdc:.cursor/skills/parallel-scheduling/SKILL.md), TSK-110): campi `token_budget`, `elapsed`, `estimated_remaining`, `bootstrap_mode`.
...
- **[code-review-protocol](mdc:.cursor/skills/code-review-protocol/SKILL.md)** (loop iterazione): decide se proseguire l'iterazione o terminare il loop.
