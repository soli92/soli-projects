...
> Adapter Cursor della skill `vcs-preflight-protocol` definita in PATTERN.md §15.

Produce uno **snapshot read-only** dello stato VCS di tutti i target della factory, per rendere
immediatamente visibile «su quale branch sto / su quale dovrei stare», con focus sui submodule
(problema dei due HEAD, PATTERN §15 §Branch Awareness Layer). Capability opt-in EP-034. Invocata
da `/vcs-status` e dal dashboard `/run`.
...
- `submodule` → `<submodule_path>`. Prima verifica init: se `<submodule_path>/.git` **non esiste**
  → stato = `NOT_INITIALIZED`, salta gli altri check per questo target.
...
Per ciascun target invoca [branch-resolver](mdc:.cursor/skills/branch-resolver/SKILL.md) (single
source of truth, R.B9) passando `resolved_vcs`, il `target_name` e — se disponibile in contesto —
il TSK corrente. In modalità snapshot generale senza TSK, `per-tsk`/`per-sprint` ritornano
`expected_branch: null` (vedi [branch-resolver](mdc:.cursor/skills/branch-resolver/SKILL.md) Step 3):
in quel caso non emettere verdict ACTION sul *naming*, ma continua a valutare
detached/NOT_INITIALIZED/drift.
...
- **detached** con branch candidato dal `--contains` → `git -C <dir> checkout <candidato>`; se
  nessun candidato → `git -C <dir> checkout -b <expected_branch>` (o il branch desiderato).
- **branch mismatch** → `git -C <dir> checkout <expected_branch>`
- **drift** → mostra i due commit e spiega: «il parent punta a X, il submodule è su Y. Per allineare
  il parent al submodule: `git -C <factory-root> add <submodule_path>` poi commit; per allineare il
  submodule al parent: `git -C <submodule_path> checkout X`». **Non scegliere automaticamente** — la
  direzione è una decisione umana.
...
- **Non è un gate**: `/vcs-status` non blocca mai un flusso. Il blocco (opt-in) vive nel gate Fase 0
  di [dev-protocol](mdc:.cursor/skills/dev-protocol/SKILL.md) (`dispatch_gate`), non qui.
- **Robustezza**: target non-git, path assente, `.gitmodules` mancante → riga con nota, mai crash
  dell'intero snapshot (best-effort per-target, isola i fallimenti).

## Cross-link

- PATTERN §15 §«Branch Awareness Layer» (R.B7, tabella inspect).
- [branch-resolver](mdc:.cursor/skills/branch-resolver/SKILL.md) (expected branch),
  [vcs-handoff](mdc:.cursor/skills/vcs-handoff/SKILL.md) (commit-time, condivide il resolver).
- Comando `/vcs-status`; integrazione dashboard: [orchestrator](mdc:.cursor/rules/orchestrator.mdc) §VCS Branch Preflight.
- ADR-EP034-001.
