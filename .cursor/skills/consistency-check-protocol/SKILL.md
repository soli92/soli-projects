...
> Adapter Cursor della skill `consistency-check-protocol` (capability EP-015, opt-in `compression.output.consistency_check.enabled`). Pattern: [[evaluator-optimizer]], [[fail-closed]].

5 step per verificare se l'output di un sub-agent contraddice il `decision_anchor`.
Invocata SOLO dall'agente [consistency-checker](mdc:.cursor/rules/consistency-checker.mdc) — **mai** invocabile da un sub-agent su se stesso (R.Q1bis).

## Step 1 — Read anchor

Leggi il campo `decision_anchor` dall'input (YAML strutturato). Valida:

- `decision_anchor.checksum` === `sha256(canonical_json(decision_anchor.decisions[]))`.
- Se mismatch → abort con ERROR `[anchor-tampered-pre-check]` in `wiki/log.md`. Nessuna verifica eseguita.
...
Leggi l'artifact referenziato da `output_artifact_ref` (read-only). Estrazioni rilevanti:
...
Verdict aggregato: il più severo tra tutti i finding.
...
- `pass`: log evento EP-013 `state: consistency_decision` con `verdict: pass`. Nessun'altra azione.
...
- [[evaluator-optimizer]]: [consistency-checker](mdc:.cursor/rules/consistency-checker.mdc) è evaluator esterno sul loop.
...
- EP-014 governor: se `checker_tokens_used > 5%` wave budget → governor escalata (coordinato cross-EP).
- Runbook: `wiki/runbooks/decision-anchor-runbook.md`.
- PATTERN §18 dominio `consistency-check`.
