...
> Adapter Cursor della skill `branch-resolver` definita in PATTERN.md §15.

Pura funzione di risoluzione: dato un target VCS (entry `code_paths` o `vcs:` legacy) e un TSK,
ritorna il **branch atteso** (`expected_branch`). È la **single source of truth** (R.B9):
[vcs-preflight-protocol](mdc:.cursor/skills/vcs-preflight-protocol/SKILL.md) (inspect) e
[vcs-handoff](mdc:.cursor/skills/vcs-handoff/SKILL.md) (commit) la invocano entrambi, così non
esistono due logiche di naming divergenti. Capability opt-in EP-034.

**Read-only**: non esegue mai comandi git che mutino lo stato. Legge config, frontmatter TSK e
opzionalmente `.factory-branches.yaml`. Non fa checkout, non crea branch.
...
- `tsk` (opzionale) — frontmatter del TSK corrente (`id`, `title`, `sprint`). Necessario solo per
  `per-tsk` / `per-sprint`. Assente quando il resolver è chiamato in modalità preflight su un target
  senza TSK in contesto (in quel caso vedi Step 3 fallback).
...
Se esiste `.factory-branches.yaml` al root del factory repo E contiene una entry per `target_name`
valida per lo sprint corrente:
...
naming storico di [vcs-handoff](mdc:.cursor/skills/vcs-handoff/SKILL.md) Fase 1 — la regola è
**spostata qui** e riusata da entrambe le skill (R.B9). Lo slug: lowercase, spazi/underscore → `-`,
rimuovi caratteri non `[a-z0-9-]`, collassa `--`, taglia a 40 caratteri.

## Step 3 — Fallback senza TSK in contesto (solo preflight)

Quando il resolver è chiamato da
[vcs-preflight-protocol](mdc:.cursor/skills/vcs-preflight-protocol/SKILL.md) su un target **senza**
un TSK corrente (snapshot generale `/vcs-status`):
...
- `per-tsk` / `per-sprint` → `expected_branch: null`, `source: strategy`,
  ...
  Il preflight in questo caso non emette verdict `ACTION` per il naming, ma segnala comunque
  detached HEAD, submodule non inizializzato e drift parent-ref.
...
- **Nessun side-effect**: non scrive file (il manifest è curato a mano o da un flusso dedicato, mai
  da questa skill).

## Cross-link

- PATTERN §15 §«Branch Awareness Layer» — regola di risoluzione + R.B9.
- Consumatori: [vcs-preflight-protocol](mdc:.cursor/skills/vcs-preflight-protocol/SKILL.md) (inspect),
  [vcs-handoff](mdc:.cursor/skills/vcs-handoff/SKILL.md) Fase 1 (commit).
